...
function PostShiftPage() {
  ...
  function publish() {
    const id = `posted-${Date.now().toString(36)}`;
    store.addPostedShift({
      id,
      title: title.trim(),
      category,
      ...
      when: when === "now" ? "Right now" : when === "today" ? "Today" : "This week",
      urgency: when,
  ...
  return (
    <PageShell>
      ...
      <div className="mx-auto max-w-3xl px-6 py-16">
            ...
            <li key={s} className="flex flex-1 items-center gap-2">
              <span
                className={`inline-flex h-7 w-7 items-center justify-center rounded-full border ${
                  i <= step ? "border-white bg-white text-black" : "border-white/20"
                }`}
              >
        ...
        {posted ? (
          <div className="rounded-3xl border border-emerald-400/30 bg-emerald-400/5 p-10 text-center">
            <div className="mx-auto inline-flex h-14 w-14 items-center justify-center rounded-full border border-emerald-400/40 bg-emerald-400/10 text-emerald-300">
              <Check className="h-6 w-6" />
            </div>
            <h2 className="mt-6 text-2xl font-light">Shift published — going live now</h2>
            <p className="mt-3 text-sm text-white/60">Redirecting you to your dashboard…</p>
          </div>
        ) : (
          <div className="rounded-3xl border border-white/10 bg-white/[0.02] p-8 md:p-10">
              ...
              <div className="space-y-6">
                ...
                <Field label="Describe it">
                  <textarea
                    ...
                    rows={5}
                    maxLength={800}
                    placeholder="What's the scope, tone, deliverables? The clearer, the faster it gets claimed."
                    className="w-full rounded-2xl border border-white/15 bg-black/40 px-4 py-3 text-base text-white placeholder:text-white/30 focus:border-white/40 focus:outline-none"
                  />
                </Field>
              </div>
                  ...
                  {(["fixed", "hourly"] as const).map((t) => (
                    <button
                      ...
                      className={`rounded-2xl border px-4 py-4 text-left transition ${
                        payType === t ? "border-white bg-white text-black" : "border-white/15 bg-white/[0.02] hover:border-white/30"
                      }`}
                    >
                      ...
                      <div className="mt-2 text-lg font-light">
                        {t === "fixed" ? "One price, one delivery" : "Pay by the hour"}
                      </div>
                    </button>
                  ))}
                </div>
                <Field label={`How much? (${payType === "fixed" ? "USD total" : "USD / hr"})`}>
                  <div className="relative">
                    ...
                    <input
                      ...
                      min={5}
                      value={pay}
                      onChange={(e) => setPay(Math.max(0, Number(e.target.value)))}
                  ...
                  <p className="mt-3 text-xs text-white/50">
                    Shifters keep 100%. Payment held in escrow the moment a shift is claimed.
                  </p>
                </Field>
              </div>
            )}
                    ...
                    {([
                      ...
                      <button
                        ...
                        className={`flex items-center justify-center gap-2 rounded-2xl border px-4 py-3 text-sm transition ${
                          when === o.v ? "border-white bg-white text-black" : "border-white/15 bg-white/[0.02] hover:border-white/30 text-white/80"
                        }`}
                      >
            ...
            <div className="mt-10 flex items-center justify-between">
              <button
                ...
                disabled={step === 0}
                className="text-xs uppercase tracking-[0.3em] text-white/50 hover:text-white disabled:opacity-30"
              >
                Back
              </button>
              {step < 3 ? (
                ...
                <button
                  onClick={publish}
                  className="inline-flex items-center gap-2 rounded-full bg-white px-6 py-3 text-xs uppercase tracking-[0.3em] text-black transition hover:bg-white/90"
                >
                  Publish shift <ArrowRight className="h-3 w-3" />
                </button>
              )}
            </div>
          </div>
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-2 block text-[10px] uppercase tracking-[0.3em] text-white/40">{label}</span>
      {children}
    </label>
  );
}
